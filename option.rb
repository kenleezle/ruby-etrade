...
class Option
  ...
  def update_price
    ...
    response = consumer.request(:get, "/market/rest/quote/#{symbol}", access_token, {"detailFlag" => "FUNDAMENTAL"})
    puts response
    sleep 0.25
    doc = REXML::Document.new response.body
    @price = doc.elements["QuoteResponse/QuoteData/all/ask"].text.to_f
    date_elem = doc.elements["QuoteResponse/QuoteData/all/dateTime"]
    @bid_ask_spread = (doc.elements["QuoteResponse/QuoteData/all/ask"].text.to_f -
                       doc.elements["QuoteResponse/QuoteData/all/bid"].text.to_f);
